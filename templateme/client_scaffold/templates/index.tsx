...
class <%= name.capitalize().pluralize() %>Page extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      tableParams: new TableParams({
        ...
        url: `/stores/${this.props.storeId}/<%= name.pluralize() %>.json`,
        filter: {
          s: ['<%= sortField %> ASC'],
        },
      }),
  ...
  handleClickDeleteItem = (record) => {
    ...
    this.setState({
      ...
      this.state.deleteActioner.do(`/stores/${this.props.storeId}/<%= name.pluralize() %>/${record.<%= apiField %>}.json`, null, {
        id: record.id,
  ...
  renderItems = () => {
    ...
    const columns = [{
      ...
      render: (value, record) => {
        return (
          <Link to={`<%= pathPrefix %>/stores/${this.props.storeId}/<%= name.pluralize() %>/${record.<%= pathField %>}`}>
            {record.<%= titleField %>}
          </Link>
        );
      },
        ...
        return (
          <div className="actions-listing">
            <Button
              ...
              onClick={
                () => {
                  this.props.dispatch(push(`<%= pathPrefix %>/stores/${this.props.storeId}/<%= name.pluralize() %>/${record.<%= pathField %>}`));
                }
              }
            />
            <Popconfirm
      ...
      <div className={styles.Container}>
        <Card className="ant-card-lg" title="<%= name.split().capitalize().pluralize() %>" id="listing">
          <New<%= name.capitalize() %>Modal
            ...
            onSuccess={
              (<%= name.camelcase() %>) => {
                this.props.dispatch(push(`<%= pathPrefix %>/stores/${this.props.storeId}/<%= name.pluralize() %>/${<%= name.camelcase() %>.<%= pathField %>}`));
              }
            }
