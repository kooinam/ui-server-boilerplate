...
class <%= name.capitalize() %>Section extends React.Component {
    ...
    this.state = {
      ...
      deleteActioner: new Actioner({
        ...
        successCallback: (<%= name.camelcase() %>) => {
          this.props.dispatch(push(`<%= pathPrefix %>/stores/${this.props.storeId}/<%= name.pluralize() %>`));
        },
  ...
  handleClickDelete = () => {
    this.state.deleteActioner.do(`/stores/${this.props.storeId}/<%= name.pluralize() %>/${this.props.<%= name.camelcase() %>.<%= apiField %>}.json`);
  }
