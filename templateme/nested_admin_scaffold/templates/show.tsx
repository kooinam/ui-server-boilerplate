...
import <%= name.capitalize() %>Section from '../admin_components/<%= name.capitalize() %>Section';
...
class <%= name.capitalize() %>Page extends React.Component {
  constructor(props) {
    ...
    this.state = {
      itemLoader: new ItemLoader({
        ...
        url: `/<%= name.pluralize() %>/${this.props.match.params.<%= routeField %>}.json`,
        callback: () => {
  ...
  handleClickTab = (tab) => {
    this.props.dispatch(push(`<%= pathPrefix %>/<%= name.pluralize() %>/${this.props.match.params.<%= routeField %>}/${tab}`));
  }
